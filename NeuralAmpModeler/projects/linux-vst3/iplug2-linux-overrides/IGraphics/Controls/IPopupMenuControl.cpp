...
{
  // OnMouseDrag() right below already guards this: the active panel is cleared
  // in CollapseEverything() while the control may still be asked to handle a
  // mouse move, and dereferencing null here took the whole host down.
  if (mActiveMenuPanel == nullptr)
  {
...
void IPopupMenuControl::CreatePopupMenu(IPopupMenu& menu, const IRECT& bounds)
{
  ...
  // GetUI() is null for a control whose delegate was set before the IGraphics
  // existed (IControl::SetDelegate() caches dlg.GetUI(), which is still null
  // while IGraphicsEditorDelegate is inside CreateGraphics()). Guard rather than
  // dereference: a null UI means there is nothing to lay the menu out against.
  if(mMaxBounds.W() == 0)
  {
    const IGraphics* pUI = GetUI();

    if(pUI == nullptr)
      return;

    mMaxBounds = pUI->GetBounds();
  }
...
void IPopupMenuControl::CalculateMenuPanels(float x, float y)
{
  ...
  for(auto i = 0; i < mActiveMenuPanel->mCellBounds.GetSize(); i++)
  {
    ...
    // Draw() maps cell `i` to item `i + mScrollItemOffset`; do the same here, or
    // a scrolled menu highlights and opens the wrong submenu. GetItem() returns
    // null past the end of the menu, which happens for the cell that Draw() uses
    // for its scroll arrow — skip those rather than dereferencing null.
    IPopupMenu::Item* pMenuItem = mActiveMenuPanel->mMenu.GetItem(i + mActiveMenuPanel->mScrollItemOffset);
...
void IPopupMenuControl::CollapseEverything()
{
  ...
  for (auto i = 0; i < mActiveMenuPanel->mCellBounds.GetSize(); i++)
  {
    ...
    if (mMouseCellBounds == pR)
    {
      ...
      // Null for the scroll-arrow cells: they are not choosable.
      if (pItem != nullptr && pItem->GetIsChoosable())
      {
...
IRECT* IPopupMenuControl::MenuPanel::HitTestCells(float x, float y) const
{
  for(auto i = 0; i < mCellBounds.GetSize(); i++)
  {
    ...
    // Same cell -> item mapping as Draw(); GetItem() is null past the end of the
    // menu (the scroll-arrow cells), and those are not cells we can hit.
    IPopupMenu::Item* pItem = mMenu.GetItem(i + mScrollItemOffset);

    if (pItem != nullptr && pItem->GetEnabled())
      return pR;
  }
  return nullptr;
}
