...
private:
  ...
  public:
    ...
    /** Largest valid first-visible-item index: the menu can only be scrolled until
     *  its last item sits in the last cell. Never negative, so that a panel whose
     *  cell list is not populated yet (or a menu with fewer items than cells)
     *  cannot push the offset below zero — Clip(v, 0, -1) returns -1, and a
     *  negative offset makes GetItem() return null for every cell. */
    int MaxScrollItemOffset() const { return std::max(0, mMenu.NItems() - mCellBounds.GetSize()); }

    void ScrollUp() { mScrollItemOffset--; mScrollItemOffset = Clip(mScrollItemOffset, 0, MaxScrollItemOffset()); }

    void ScrollDown() { mScrollItemOffset++; mScrollItemOffset = Clip(mScrollItemOffset, 0, MaxScrollItemOffset()); }
