...
{
  // Stop the idle timer before any member of this object (or of the plugin
  // class derived from it) is destroyed. On macOS/Windows the timer callback
  // runs on the main/UI thread, so it cannot overlap destruction; on Linux it
  // runs on its own pthread, and IPlugAPIBase's destructor — which is what
  // normally stops it — only runs after every derived member is already gone.
  // Without this, the 20ms thread can be inside OnTimer() -> OnIdle() touching
  // destroyed UI state while the object is being torn down.
  // IPlugVST3Controller is a friend of IPlugAPIBase, so mTimer is reachable.
  if (mTimer)
    mTimer->Stop();
}
